 ...
 */
ProjectionData load_projection_data(int projection_id, int num_voxels, const std::string &input_dir, int projections) {
    std::string voxel_dir = input_dir + "/" + std::to_string(num_voxels);
    ProjectionData data;

    // Load 2D projection data
    data.projection = read_file(detector_rows * detector_columns *projections, projection_id * detector_rows * detector_columns,
                                input_dir + "/projections.bin");

    // Load transform matrix used to align the 3D volume position towards the recorded 2D projection
    data.transform_matrix = read_file(3 * 4 *projections, projection_id * 3 * 4, input_dir + "/transform.bin");

    // Load volume weight used to compensate for cone beam ray density
    data.volume_weight = read_file(num_voxels * num_voxels *projections, projection_id * num_voxels * num_voxels,
